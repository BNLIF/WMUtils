...
This repository was originally forked from https://github.com/AIT-WATCHMAN/WMUtils in Feb 2022. The watchmanInstaller.sh will downloaded a fresh copy of cmake, python, geant4, ratpac, ... and build them from scratch. The installation will result a 10 GB WMUtils directory. Thus, we do not want to do multiple installations on the same machine (ex. dune0001.sdcc.bnl.gov). Instead, we have a shared group directory at /gpfs01/lbne/users/wbls where this WMUtils is installed. People are encourage to develop ratpac on their own home directory under /gpfs01/lbne/users, and compile ratpac on their own by linking the dependence in this WMUtils. A separate ratpac repository is forked into BNLIF/WbLS as well, and we should be used that one to corrodinate separate code development.
